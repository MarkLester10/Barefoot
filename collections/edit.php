...
<main class="xl:flex-1 xl:overflow-x-hidden">
  ...
  <section class="blog__collections py-6 px-4 xl:px-0 xl:pr-4">
    ...
    <h1 class="title__text text__adaptive">Edit Story</h1>
    <form class="mixitup grid grid-cols-1 lg:grid-cols-2 gap-6 py-8" action="edit.php" method="post"
      enctype="multipart/form-data">
      <input type="hidden" name="post_id" value="<?php echo $editingStory['id'] ?>">
      <input type="hidden" name="image" value="<?php echo $editingStory['image'] ?>">
      ...
      <div class="blog__details py-6 bg__adaptive px-4 shadow-md">
        ...
        <label class=" block mt-4 text-sm">
          ...
          <select id="tags" name="tags[]" class="mt-1" multiple>
            ...
            <?php foreach ($editingStoryTags as $editingStoryTag) : ?>
            <?php if ($tag['name'] === $editingStoryTag['name']) : ?>
      ...
      <div class="blog__image rounded-md relative">
        <label class="text-sm cursor-pointer">
          <img id="imageDisplay" src='<?php echo BASE_URL . "/assets/imgs/travels/{$image}" ?>'
